...
class Slider extends Component {
  ...
  handleTouchMove = (e) => {
    ...
    const diffX =
      (this.touchStartPosition.clientX - e.changedTouches[0].clientX) * -1;
  ...
  handleTouchEnd = (e) => {
    ...
    if (!this.canNavigateBackward && !isForwardNavigation)
      return this.resetTransition({ animated: true });
    if (!this.canNavigateForward && isForwardNavigation)
      return this.resetTransition({ animated: true });
  ...
  render() {
    ...
    const wrapperProps = {
      className: `${_.slider} ${className} ${
        showSlideButtons && _.hasSlideButtons
      }`,
        ...
        {showSlideButtons && (
          <div>
            {this.canNavigateBackward && (
              <button
                className={`${_.prevNextButton} ${_.prev}`}
            ...
            {this.canNavigateForward && (
              <button
                className={`${_.prevNextButton} ${_.next}`}
